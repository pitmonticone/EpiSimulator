...
mutable struct Patient <: AbstractAgent
    id::Int           # identity code ∈ ℕ
   	...
   	residence::Int    # household node ∈ ℕ || RESIDENCE!!
...
function contact!(agent, model,location)
	...
	for age_group in 1:model.K
		...
		ncontacts = round(Int, LightGraphs.weights(contact_graph)[age_group, agent.age_group]) # in
...
function initialize_model(provincial_pops::Array{Int,1}, age_provincial_pops::DataFrame,
		home_contact_graph::SimpleWeightedDiGraph{Int64,Float64}, work_contact_graph::SimpleWeightedDiGraph{Int64,Float64}, school_contact_graph::SimpleWeightedDiGraph{Int64,Float64}, other_contact_graph::SimpleWeightedDiGraph{Int64,Float64}, mobility_graph::SimpleWeightedDiGraph{Int64,Float64},h::Real,I0::Int)
	...
	properties = @dict(home_contact_graph, work_contact_graph, school_contact_graph, other_contact_graph,
	...
	for province ∈ 1:M
		for age ∈ 1:K
			for n ∈ 1:age_provincial_pops[province][age]
				...
				delay_left=-1
				diagnosis=:O
				add_agent!(pos, model, age_group, residence, status,
...
model = initialize_model(provincial_pops, age_provincial_pops, home_contact_graph, work_contact_graph, school_contact_graph, other_contact_graph, mobility_graph,h, I0)
